...
import { decodeFlac } from "../engine/decode.mjs?v=1.2.0";
import { decodeWav, encodeWav } from "../engine/wav.mjs?v=1.2.0";
import { encodeFlac } from "../engine/flac-encode.mjs?v=1.2.0";
import { encodeMp3 } from "../engine/mp3-encode.mjs?v=1.2.0";
import { analyze } from "../engine/detect.mjs?v=1.2.0";
import { loadRubberBand, pitchShiftOffline, peakOf, applyPeakSafety } from "../engine/correct.mjs?v=1.2.0";

let rbApiPromise = null;
function getRubberBand() {
  if (!rbApiPromise) {
    rbApiPromise = fetch(new URL("../vendor/rubberband-wasm/rubberband.wasm?v=1.2.0", import.meta.url))
