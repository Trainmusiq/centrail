...
14. **Ko-fi visible siempre (registrada 16 jul 2026, copy actualizado 28 jul 2026)**: el footer debe mostrar el enlace a ko-fi.com/trainmusiq con el copy canónico bilingüe (ver `roadmap.md` §3.12 de `trainmusiq/trainmusiq`) — agradece si la herramienta sirvió, sin prometer tiers que aún no existen. El reemplazo del canon en el repo padre está escrito y listo en `docs/canon-copy-kofi.md`; borrar ese archivo una vez aplicado allá.
15. **El copy nunca acusa a la propia medición (registrada 28 jul 2026)**: se describe el material, jamás se califica la medición de "no confiable" ni equivalentes. Un intervalo ancho que no cruza el cero sigue siendo una conclusión sólida sobre la dirección del desvío, y así se comunica (§4.3 de la spec). Veredicto y panel de deriva cuentan la MISMA historia con el MISMO tono sobre el mismo archivo.
...
- **`el.style.display = ""` NO significa "visible"** si el CSS ya define un `display` distinto para ese selector (ej. `#modeLive { display: none; }` como estado base) — `""` solo borra el override inline y cae de vuelta a la regla del stylesheet, que puede seguir siendo `none`. Costó una vuelta de debugging en v1.3 (22 jul): el contenido SÍ se renderizaba en el DOM pero quedaba invisible. Cuando un contenedor tiene un `display:none` de base en CSS, el JS que lo muestra debe asignar un valor concreto (`"block"`, `"flex"`, etc.), nunca `""`.
- **El bump de cache-busting solo reemplazaba la versión ANTERIOR, así que un archivo rezagado nunca se ponía al día** (detectado 28 jul 2026 con el grep de huérfanos del cierre de v1.3.3): `engine/i18n.mjs` arrastró `?v=1.2.1` durante **tres releases**, sirviendo los diccionarios de idioma con cache-busting obsoleto — justo el archivo cuyo contenido más cambia. Corregido en `scripts/bump-cache-version.sh`: ahora el sed reemplaza **cualquier** `?v=<semver>` y el script **falla** si queda alguna referencia en otra versión. Correr igual el grep de huérfanos al cerrar (`grep -rn "?v=" index.html engine/*.mjs workers/*.mjs | grep -v "?v=<nueva>"`).
- **Navegar solo cambiando el hash (`#/ruta`) NO recarga el documento** (comportamiento estándar del navegador: mismo documento, mismo origen, solo dispara `hashchange`) — si se edita el HTML/JS fuente y se re-navega a una URL que solo difiere en el fragmento, el navegador reutiliza el documento ya cargado en memoria y los cambios no se ven. Hace falta un reload real (`location.reload()` o navegar primero a la URL sin hash) para probar cambios tras editar código de ruteo por hash.
